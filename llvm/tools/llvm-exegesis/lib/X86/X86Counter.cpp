...
static bool CheckLbrFormat(const struct perf_event_mmap_page &Page) {
  static const uint64_t mask = 0x000000000000003F;
  // We are looking for the format that contains "cycles".
  // (Intel SDM, vol 3B 17.4.8.1)
  // "MSR IA32_PERF_CAPABILITIES[5:0]" == ***110B

  // The type of perf_event_mmap_page::capabilities is union of {unit64_t,
  // and a struct with 64 bits}. But you're allowed to read common prefix
  // regardless of which union member is active.
  // So it should be safe to just access the `unit64_t capabilitites`
  // member.
  return (Page.capabilities & mask) & 6;
}

llvm::Expected<int64_t> X86LbrCounter::readOrError() const {
  ...
  // Check to see if the LBR format is expected.
  // FIXME: It would be nicer if we could detect the format way earlier.
  ...
  if (!error)
    ...
    // because We should report the cycles count for all jumps, not just the
    // most recent.
    return CycleArray[0];
  return error;
}
